...
Simplicity & Ease of Use — Easy to understand, design, and use.

Stateless — Each API call is independent; no session/data stored on server between requests.

Scalability — Supports load balancing; easily scalable system.

Flexible Data Formats — JSON/XML/HTML/text.

Uniform Interface — Clear standard rules (HTTP verbs, status codes).

Caching — Improves performance for non-changing data (like static menu or catalog).

Separation of Concerns — Frontend and backend can be built in different tech stacks.

Interoperability — Language agnostic — clients and servers can be in different languages.

Easy Testing & Debugging — Tools like Postman help in API validation.

Security — Supports various authentication mechanisms (API keys, OAuth, JWT).
...
Decide endpoints — what resources (e.g., /users, /products)?

Choose HTTP methods — GET/POST/PUT/DELETE.

Design response format (JSON recommended).

Handle error codes properly.

Secure with authentication (API key, OAuth, JWT).
...
Faster, binary framing

Low latency, built for mobile
...
Note: Drawn diagrams and high-level concepts make it easier to communicate system design in frontend interviews for Senior Developers.


Uploading a website using FileZilla or WinSCP.
